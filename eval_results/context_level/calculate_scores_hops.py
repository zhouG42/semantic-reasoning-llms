...
def read_lists_from_file(file_path):
    ...
    try:
        with open(file_path, 'r') as file:
            for line in file:
                # strip any leading/trailing whitespace and parse the line as a list
...
def calculate_metrics(ground_truth_path, model_output_path):
    ...
    for gt_list, model_list in zip(ground_truth_lists, model_output_lists):
        ...
        # convert lists to binary lables of 0/1
    ...
    for model in ['llama','gpt','qwen']:
        for rule in ['no','nl','jena']:
            for hop in ['direct','indirect']:
                ground_truth_path = f"./groundtruth/gt_{type}_{hop}.txt"
                for run in range(3):
                    model_output_path = f"./three_runs/LLM_outputs/{model}/{type}/{model}_{rule}_{hop}_{run+1}.txt"
                    print("Current file being calcluated: ",model_output_path)
                    metrics = calculate_metrics(ground_truth_path, model_output_path)

                    file_path = f"./scores/scores_{type}/three_runs/scores_{model}_{rule}_{hop}.txt"
